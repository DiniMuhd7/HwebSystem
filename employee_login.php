...
{   
    ...
    mysqli_select_db($database, "ehospital") or die("Could not select database");   
        ...
        $role = trim($_POST['role']);
 		...
 		$qry = "SELECT `employee_type` FROM `employee` WHERE `employee_email` = '$email' AND employee_password = '$password' AND employee_type = '$role'";
        $result = mysqli_query($database, $qry);
        
        if(!$result || mysqli_num_rows($result) <= 0)
        { 
            echo "The email address, password or role does not match.";
            return "The email address, password or role does not match.";
        }
        else if ($role == "physician"){
        	$row = mysqli_fetch_assoc($result);
        	$fgmembersite->RedirectToURL("phycisian_home.php");
        	header("location: ./phycisian_home.php");

        } else if ($role == "pharmacy"){
        	$row = mysqli_fetch_assoc($result);
        	$fgmembersite->RedirectToURL("phamacist_home.php");
        	header("location: ./phamacist_home.php");

        } else if ($role == "lab scientist"){
        	$row = mysqli_fetch_assoc($result);
        	$fgmembersite->RedirectToURL("lab_scientist_home.php");
        	header("location: ./lab_scientist_home.php");
	...
	<input type='hidden' name='submitted' id='submitted' value='1'/>
  		...
  		<div class = "group">
  			<select name='role' id='role' required >
                ...
                <option class="yellow-text text-darken-2" name="pharmacy" id="pharmacy" value="pharmacy">pharmacy</option>
                <option class="yellow-text text-darken-2" name="physician" id="physician" value="physician">physician</option>
                <option class="yellow-text text-darken-2" name="lab_scientist" id="lab_scientist" value="lab scientist">lab scientist</option>
            </select>
        </div>
