...
<body>
	...
	<header class="header">
		<a class = "head-title" href="./index.php"><strong>LabScientist Home</strong></a>
	...
	<input type='hidden' name='submitted' id='submitted' value='1'/>
    	...
    	<div class="ripples buttonRipples"><span class="ripplesCircle"></span></div>
  		...
  		<a class = 'head-title' type='submit' href='./update_medical.php' name='edit' id='edit' class="">EDIT RECORD</a>
  		<br>
...
{ 
   ...
   if($patientId ==""||$patientId==null){
	  $query = "SELECT `patient`.`patient_id`, `patient`.`name`, `patient`.`lastname`, `patient`.`gender`, `patient`.`email`, `patient`.`birthday`, `prescription`.`prescription_id`, `prescription`.`prescription_title`,`prescription`.`prescription_date`, `employee`.`employee_lname`, `employee`.`employee_type` FROM `prescription` INNER JOIN `patient` ON `prescription`.`patient_id` = `patient`.`patient_id` INNER JOIN `employee` ON `prescription`.`employee_id` = `employee`.`employee_id`";
	}
	else {
	  $query = "SELECT `patient`.`patient_id`, `patient`.`name`, `patient`.`lastname`, `patient`.`gender`, `patient`.`email`, `patient`.`birthday`, `prescription`.`prescription_id`, `prescription`.`prescription_title`,`prescription`.`prescription_date`, `employee`.`employee_lname`, `employee`.`employee_type` FROM `prescription` INNER JOIN `patient` ON `prescription`.`patient_id` = `patient`.`patient_id` INNER JOIN `employee` ON `prescription`.`employee_id` = `employee`.`employee_id` WHERE `prescription`.`patient_id` = '$patientId';";
	}
